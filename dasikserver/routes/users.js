...
router.post('/findUser', async function(req, res, next) {
  ...
  var user = await User.find({name:username},{"_id":false, "__v":false, monthDiet:{"_id":false, breakFast:{"_id":false }, lunch:{"_id":false}, dinner:{"_id":false}}});
...
router.post('/saveUserInfo', async function(req,res,next){
  var body = req.body;
  console.log(body);
  var user = await User.find({name:body.name},{"_id":false, "__v":false, monthDiet:{"_id":false}});
  ...
  if (user.length == 0){
    ...
    await User.update({name:body.name},{height:body.height, weight: body.weight, allergy: body.allergy, checkMeal: body.checkMeal, monthDiet:body.monthDiet})
  }
